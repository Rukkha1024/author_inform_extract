...
Author Profile URL을 입력하면 모든 논문 정보를 JSON으로 추출합니다.
...
def save_json_result(json_content, output_dir, author_url):
    """JSON 결과를 파일로 저장"""
...
def main_cli():
    ...
    parser = argparse.ArgumentParser(
        description="Google Scholar 저자의 모든 논문 정보를 JSON 형식으로 추출합니다.",
    ...
    # 논문 정보 추출 실행
    try:
        json_result = main(author_url)

        if not json_result or '"papers"' not in json_result:
            print("\n⚠ 경고: 추출된 논문 정보가 없습니다.")
            sys.exit(1)

        # 결과 저장
        output_path = save_json_result(json_result, args.output, author_url)
